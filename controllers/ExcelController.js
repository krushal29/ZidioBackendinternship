...
const uploadExcelFile = async (req, res) => {
    ...
    try {
        ...
        const ProfileFile = uploadFile.secure_url;
        fs.unlinkSync(Files); // Clean up local file
        ...
        return res.json({ success: true, ProfileFile, asset_id: uploadFile.asset_id, display_name: req.file.originalname });
...
const ExcelAllData = async (req, res) => {
  try {
    ...
    console.error("ExcelAllData Error:", e); // Add helpful logs
    return res.status(500).json({ data: false, message: "Server Error. Please try again!", error: e.message });
  }
};
